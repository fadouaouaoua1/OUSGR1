...
<div class="container mx-auto px-6 py-8">
    <div class="bg-white shadow-md rounded px-6 py-8">
        <h1 class="text-2xl font-semibold text-gray-800 mb-6">Ajouter un SLA</h1>

        <form method="POST" action="{{ route('admin.slas.store') }}">
            @csrf
            ...
            <div class="mb-4">
                ...
                <input id="titre" name="titre" type="text" value="{{ old('titre') }}"
                       ...
                       placeholder="Ex. : SLA pour réclamation urgente">
            </div>

            <div class="mb-6">
                <label for="delai_heures" class="block text-sm font-medium text-gray-700 mb-1">Délai (en heures)</label>
                <input id="delai_heures" name="delai_heures" type="number" min="1" value="{{ old('delai_heures') }}"
                       class="w-full px-4 py-2 border rounded-md shadow-sm focus:ring focus:ring-blue-100"
                       placeholder="Ex. : 48">
            </div>
